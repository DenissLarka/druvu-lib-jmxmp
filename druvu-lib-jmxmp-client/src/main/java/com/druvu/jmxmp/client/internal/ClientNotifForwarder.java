...
public abstract class ClientNotifForwarder {
    ...
    public synchronized void addNotificationListener(
        ...
        infoList.put(
                listenerID, new ClientListenerInfo(listenerID, name, listener, filter, handback, delegationSubject));
    ...
    public synchronized ClientListenerInfo[] preReconnection() throws IOException {
        ...
        final ClientListenerInfo[] tmp =
                (ClientListenerInfo[]) infoList.values().toArray(new ClientListenerInfo[0]);
    ...
    /**
     * Called after reconnection is finished. This method is intended to be called only by a client connetor: <code>
     * RMIConnector</code/> and <code/>ClientIntermediary</code>.
     */
    ...
    private class NotifFetcher implements Runnable {
        ...
        private NotificationResult fetchNotifs() {
            try {
                NotificationResult nr =
                        ClientNotifForwarder.this.fetchNotifs(clientSequenceNumber, maxNotifications, timeout);
                ...
                if (!shouldStop()) {
                    logger.error(
                            "NotifFetcher-run", "Failed to fetch notification, stopping thread. Error is: " + ioe, ioe);
        ...
        */
        private NotificationResult fetchOneNotif() {
            ...
            while (result == null && !shouldStop()) {
                ...
                try {
                    ...
                    if (e instanceof ClassNotFoundException || e instanceof NotSerializableException) {
                        logger.warning(
                                "NotifFetcher.fetchOneNotif", "Failed to deserialize a notification: " + e.toString());
                        if (logger.traceOn()) {
            ...
            if (notFoundCount > 0) {
                final String msg = "Dropped " + notFoundCount + " notification" + (notFoundCount == 1 ? "" : "s")
    ...
    /**
     * Import: should not remove a listener dureing reconnection, the reconnection needs to change the listener list and
     * that will possibly make removal fail.
     */
    ...
    /**
     * This variable is used to tell whether a connector (RMIConnector or ClientIntermediary) is doing reconnection.
     * This variable will be set to true by the method <code>preReconnection</code>, and set fase by <code>
     * postReconnection</code>. When beingReconnected == true, no thread will be created for fetching notifications.
     */
