...
public class EnvHelp {
     ...
     *
     * <ul>
     *   <li>The ClassLoader object found in <var>env</var> for <tt>jmx.remote.default.class.loader</tt>, if any.
     *   <li>The ClassLoader pointed to by the ObjectName found in <var>env</var> for
     *       <tt>jmx.remote.default.class.loader.name</tt>, and registered in <var>mbs</var> if any.
     *   <li>The current thread's context classloader otherwise.
     * </ul>
     *
     ...
     *     <ul>
     *       <li>both <tt>jmx.remote.default.class.loader</tt> and <tt>jmx.remote.default.class.loader.name</tt> are
     *           specified,
     *       <li>or <tt>jmx.remote.default.class.loader</tt> is not an instance of {@link ClassLoader},
     *       <li>or <tt>jmx.remote.default.class.loader.name</tt> is not an instance of {@link ObjectName},
     *       <li>or <tt>jmx.remote.default.class.loader.name</tt> is specified but <var>mbs</var> is null.
     * @throws InstanceNotFoundException if <tt>jmx.remote.default.class.loader.name</tt> is specified and the
     *     ClassLoader MBean is not found in <var>mbs</var>.
     */
    public static ClassLoader resolveServerClassLoader(Map env, MBeanServer mbs) throws InstanceNotFoundException {
        ...
        if (loader != null && name != null) {
            final String msg = "Only one of " + DEFAULT_CLASS_LOADER
                    + " or " + DEFAULT_CLASS_LOADER_NAME
                    + " should be specified.";
     ...
     * <ul>
     *   <li>The ClassLoader object found in <var>env</var> for <tt>jmx.remote.default.class.loader</tt>, if any.
     *   <li>The <tt>Thread.currentThread().getContextClassLoader()</tt> otherwise.
     * </ul>
     *
     ...
     * @throws IllegalArgumentException if <tt>jmx.remote.default.class.loader</tt> is specified and is not an instance
     *     of {@link ClassLoader}.
     */
    ...
    // EnvHelp.initCause / EnvHelp.getCause removed: Throwable.initCause /
    // Throwable.getCause are native since J2SE 1.4. Call sites now use the
    // chained Throwable constructor or t.initCause(cause) / t.getCause()
    // directly.
    ...
    public static int getNotifBufferSize(Map env) {
        ...
        try {
            if (env.containsKey(BUFFER_SIZE_PROPERTY)) {
                queueSize = (int)
                        EnvHelp.getIntegerAttribute(env, BUFFER_SIZE_PROPERTY, defaultQueueSize, 0, Integer.MAX_VALUE);
            } else { // try the old one
     ...
     * associated with this attribute should be a <code>Long</code> object. The default value is 60000 milleseconds.
     */
     ...
     * or it may be a String that is parsable as a long. It must be at least <code>minValue</code> and at most<code>
     * maxValue</code>.
     *
    ...
    public static final String DEFAULT_HIDDEN_ATTRIBUTES = "java.naming.security.* " + "jmx.remote.authenticator "
            + "jmx.remote.context "
