 ...
 *
 * <p>A typical use of this class is to insert it between a connector server such as the RMI connector and the
 * MBeanServer with which the connector is associated. Requests from the connector client can then be filtered and those
 * operations that are not allowed, or not allowed in a particular context, can be rejected by throwing a <code>
 * SecurityException</code> in the corresponding <code>check*</code> method.
 *
 * <p>This is an abstract class, because in its implementation none of the checking methods does anything. To be useful,
 * it must be subclassed and at least one of the checking methods overridden to do some checking. Some or all of the
 * MBeanServer methods may also be overridden, for instance if the default checking behaviour is inappropriate.
 *
 * <p>If there is no SecurityManager, then the access controller will refuse to create an MBean that is a ClassLoader,
 ...
 * are presumably advanced enough to handle policy files and security managers.
 */
    ...
    private void checkMLetAddURL(ObjectName name, String operationName) throws InstanceNotFoundException {
        ...
        if (sm == null) {
            if (operationName.equals("addURL")
                    && getMBeanServer().isInstanceOf(name, "javax.management.loading.MLet")) {
                throw new SecurityException("Access denied! MLet method " + "addURL cannot be invoked "
