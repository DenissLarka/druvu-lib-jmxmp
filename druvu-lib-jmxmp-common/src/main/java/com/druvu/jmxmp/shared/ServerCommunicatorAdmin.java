...
public abstract class ServerCommunicatorAdmin {
    public ServerCommunicatorAdmin(long timeout) {
        if (logger.traceOn()) {
            logger.trace(
                    "Constructor", "Creates a new ServerCommunicatorAdmin object " + "with the timeout " + timeout);
        }
    ...
    /**
     * Tells that a new request message is received. A caller of this method should always call the method <code>
     * rspOutgoing</code> to inform that a response is sent out for the received request.
     *
     * @return the value of the termination flag:
     *     <ul>
     *       <code>true</code> if the connection is already being terminated, <br>
    ...
    private static final ClassLogger logger =
            new ClassLogger("javax.management.remote.misc", "ServerCommunicatorAdmin");
    private static final ClassLogger timelogger =
            new ClassLogger("javax.management.remote.timeout", "ServerCommunicatorAdmin");
}
