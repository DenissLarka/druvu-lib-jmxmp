 ...
 *
 * <p>Each profile is created by an instance of {@link ProfileServerProvider}. This instance is found as follows.
 * Suppose the given <code><em>profile</em></code> looks like <code>TLS</code>. Then the factory will attempt to find
 * the appropriate {@link ProfileServerProvider} for <code><em>tls</em></code>. Suppose the given <code><em>profile</em>
 * </code> looks like <code>SASL/PLAIN</code>. Then the factory will attempt to find the appropriate
 * {@link ProfileServerProvider} for <code><em>sasl</em></code>. The <code><em>profile</em></code> string passed in to
 * the factory is converted into lowercase and all the characters after the <code>/</code> character are discarded.
 *
 * <p>A <em>provider package list</em> is searched for as follows:
 *
 * <ol>
 *   <li>If the <code>environment</code> parameter to {@link #createProfile(String, Map) createProfile} contains the key
 *       <code>jmx.remote.profile.provider.pkgs</code> then the associated value is the provider package list.
 ...
 * list is not a String, or if it contains an element that is an empty string, a {@link ProfileProviderException} is
 * thrown.
 *
 ...
 * </blockquote>
 *
 * <p>If the <code>environment</code> parameter to {@link #createProfile(String, Map) createProfile} contains the key
 * <code>jmx.remote.profile.provider.class.loader</code> then the associated value is the class loader to use to load
 * the provider. If the associated value is not an instance of {@link java.lang.ClassLoader}, an
 * {@link java.lang.IllegalArgumentException} is thrown.
 *
 * <p>If the <code>jmx.remote.profile.provider.class.loader</code> key is not present in the <code>environment</code>
 * parameter, the class loader that loaded the <code>ProfileServerFactory</code> class is used.
 *
 ...
 *
 * <p>Otherwise, a problem with the found provider is signalled by a {@link ProfileProviderException} whose
 * {@link ProfileProviderException#getCause() <em>cause</em>} indicates the underlying exception, as follows:
 *
 * <ul>
 ...
 * then the implementation will use its own provider for <code><em>profile</em></code>, or it will throw a <code>
 * IllegalArgumentException</code> if there is none.
 *
 * <p>Once a provider is found, the result of the <code>createProfile</code> method is the result of calling
 * {@link ProfileServerProvider#createProfile(String, Map) createProfile} on the provider.
 *
 * <p>The <code>Map</code> parameter passed to the <code>ProfileServerProvider</code> is a new read-only copy of the
 * <code>environment</code> parameter to {@link #createProfile(String, Map) ProfileServerFactory.createProfile}, or an
 * empty <code>Map</code> if that parameter is null. If the <code>jmx.remote.profile.provider.class.loader</code> key is
 * not present in the <code>environment</code> parameter, it is added to the new read-only <code>Map</code>. The
 * associated value is the class loader that loaded the <code>ProfileServerFactory</code> class.
 */
public class ProfileServerFactory {
    ...
    private static final String PROFILE_PROVIDER_DEFAULT_PACKAGE = "com.druvu.jmxmp.server.profile";
     ...
     *     this map must be Strings. The appropriate type of each associated value depends on the attribute.
    ...
    private static final ProfileServerProvider getProvider(String profile, String pkgs, ClassLoader loader)
        ...
        while (tokenizer.hasMoreTokens()) {
            ...
            if (!(obj instanceof ProfileServerProvider)) {
                final String msg = "Provider not an instance of " + ProfileServerProvider.class.getName() + ": "
    ...
    private static final ClassLoader resolveClassLoader(Map environment) {
        ...
        if (environment != null) {
            try {
                ...
                final String msg = "ClassLoader not an instance of java.lang.ClassLoader : "
